...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 8, 3, padding=0, bias=False),  # 28x28 → 28x28, RF=3
            nn.BatchNorm2d(8),
            nn.ReLU()
        )
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(8, 16, 3, padding=0, bias=False),  # 28x28 → 28x28, RF=5
            nn.BatchNorm2d(16),
            nn.ReLU()
        )

        self.conv3 = nn.Sequential(
            nn.Conv2d(16, 8, 3, padding=0, bias=False),  # 28x28 → 28x28, RF=7
            nn.BatchNorm2d(8),
            nn.ReLU()
        )

        # POOLING 1
        self.pool1 = nn.MaxPool2d(2,2)  # 28x28 → 14x14, RF=8

        # CONV BLOCK 2
        self.conv4 = nn.Sequential(
            nn.Conv2d(8, 16, 3, padding=0, bias=False),  # 14x14 → 14x14, RF=10
            nn.BatchNorm2d(16),
            nn.ReLU()
        )

        self.conv5 = nn.Sequential(
            nn.Conv2d(16, 8, 3, padding=0, bias=False),  # 14x14 → 14x14, RF=12
            nn.BatchNorm2d(8),
            nn.ReLU()
        )

        self.conv6 = nn.Sequential(
            nn.Conv2d(8, 16, 3, padding=0, bias=False),  # 14x14 → 14x14, RF=14
            nn.BatchNorm2d(16),
            nn.ReLU()
        )
        # FINAL CONV BLOCK
        self.conv7 = nn.Sequential(
            nn.Conv2d(16, 8, 3, padding=0, bias=False),  # 7x7 → 5x5, RF=28
            nn.BatchNorm2d(8),
            nn.ReLU()
        )

        # GAP and output
        self.gap = nn.AdaptiveAvgPool2d(1)  # 5x5 → 1x1
        self.conv8 = nn.Conv2d(8, 10, 1, bias=False)  # 1x1 → 10 classes
